...
interface DealCardProps {
  deal: Deal;
  /**
   * Renders the lapsed treatment. Listings filter expired deals out entirely,
   * so this is only set where a deal is shown deliberately — the bookmarks
   * page, where silently dropping a saved coupon would be more confusing than
   * showing it as ended.
   */
  expired?: boolean;
}

export default function DealCard({ deal, expired = false }: DealCardProps) {
  ...
  return (
    <Link href={`/deals/${deal.id}`} className="block">
      <article className={`glass-card rounded-2xl overflow-hidden h-full flex flex-col p-5 relative border border-slate-200/40 dark:border-slate-800/40 ${expired ? "opacity-60" : ""}`}>
        {/* Top Badges / Actions */}
        <div className="flex items-center justify-between mb-4">
          {expired ? (
            <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-slate-200 text-slate-500 dark:bg-slate-800 dark:text-slate-400">
              종료됨
            </span>
          ) : (
        ...
        <div className="mt-auto space-y-3">
          <div className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500">
            ...
            <span>
              {deal.validUntil ? `기한: ${deal.validUntil}까지` : "상시 진행"}
            </span>
          </div>
          ...
          {expired ? (
            <div className="w-full text-center text-xs font-bold px-3 py-2.5 rounded-xl bg-slate-200 text-slate-500 dark:bg-slate-800 dark:text-slate-400">
              {deal.validUntil ? `${deal.validUntil}자로 종료` : "종료된 혜택"}
            </div>
          ) : (
            <CopyAndGoButton
              code={deal.code}
